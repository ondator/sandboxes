...
if [ ! -e "$FIRST_START_DONE" ]; then
  kdb5_ldap_util -D cn=admin,dc=example,dc=org create -subtrees ou=people,dc=example,dc=org:ou=tuz,dc=example,dc=org -sscope sub -r EXAMPLE.ORG -s -H ldap://ldap -P password -w admin
